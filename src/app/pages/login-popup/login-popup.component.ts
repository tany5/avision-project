...
export class LoginPopupComponent implements OnInit {
  ...
  constructor(private formbuilder: FormBuilder,private headerService: HeaderService,private route: ActivatedRoute,private router: Router, @Inject(PLATFORM_ID) private platformId: Object,private dialogref: MatDialogRef<LoginPopupComponent>,private localStorage: LocalstorageService) {
    ...
    (data:any) => {
      ...
      if(data.status == 200){
        ...
        if (isPlatformBrowser(this.platformId)) {
          ...
          this.localStorage.setItem('currentUser', 'true');
          this.localStorage.setItem('currentUserSession','true');
          this.localStorage.setItem('currentUserId',data.user_id);
          this.localStorage.setItem('currentUserName',data.user_information.user_name);
          this.localStorage.setItem('currentUserEmail',data.user_information.user_email);
          this.localStorage.setItem('currentUserEmail',data.user_information.user_phone);
          this.localStorage.setItem('userloggedIn','1');
  ...
  this.headerService.login(login_form.value.login_email,login_form.value.login_password).subscribe(
    ...
    (data:any) => {
      console.log("user information...");
        ...
        if(data.status == 200){
          if (isPlatformBrowser(this.platformId)) {
            this.localStorage.setItem('currentUser', '1');
          this.localStorage.setItem('currentUserSession','true');
          this.localStorage.setItem('currentUserId',data['user_id']);
          this.localStorage.setItem('currentUserName',data['user_information'][0]['user_name']);
          this.localStorage.setItem('currentUserEmail',data['user_information'][0]['user_email']);
          this.localStorage.setItem('currentUserEmail',data['user_information'][0]['user_phone']);
          this.localStorage.setItem('userloggedIn','1');
...
freeliveclass(){
  if(this.localStorage.getItem('userloggedIn') == '1'){
    this.router.navigate(['dashboard/videos'])
  }else{
